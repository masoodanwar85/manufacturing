...
@section('content')
	<div class="card card-default color-palette-box">
		...
		<div class="card-body">
			<form class="form-horizontal" action="{{ route('bankAccount.store') }}" method="POST">
				@csrf
					...
					<div class="col-sm-10">
						<select name="bankID" class="form-control select2 @if($errors->has('bankID')) is-invalid @endif" required>
							...
							@foreach($banks as $bank)
								<option value="{{ $bank->bankID }}" {{ old('bankID') == $bank->bankID ? 'selected' : '' }}>{{ $bank->bankName }}</option>
							@endforeach
						</select>
				...
				<div class="form-group row {{ $errors->has('accountTitle') ? 'has-error' : '' }}">
					...
					<div class="col-sm-10">
						<input type="text" name="accountTitle" class="form-control @if($errors->has('accountTitle')) is-invalid @endif" value="{{ old('accountTitle') }}" required>
				...
				<div class="form-group row {{ $errors->has('accountNumber') ? 'has-error' : '' }}">
					...
					<div class="col-sm-10">
						<input type="text" name="accountNumber" class="form-control @if($errors->has('accountNumber')) is-invalid @endif" value="{{ old('accountNumber') }}" required>
				...
				<div class="form-group row {{ $errors->has('branchCode') ? 'has-error' : '' }}">
					...
					<div class="col-sm-10">
						<input type="number" name="branchCode" class="form-control @if($errors->has('branchCode')) is-invalid @endif" value="{{ old('branchCode') }}" required>
				...
				<div class="form-group row {{ $errors->has('branchName') ? 'has-error' : '' }}">
					...
					<div class="col-sm-10">
						<input type="text" name="branchName" class="form-control @if($errors->has('branchName')) is-invalid @endif" value="{{ old('branchName') }}" required>
				...
				<div class="form-group row {{ $errors->has('branchLocation') ? 'has-error' : '' }}">
					...
					<div class="col-sm-10">
						<input type="text" name="branchLocation" class="form-control @if($errors->has('branchLocation')) is-invalid @endif" value="{{ old('branchLocation') }}" required>
