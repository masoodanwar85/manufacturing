...
@section('content')
	<div class="card card-default color-palette-box">
		<div class="card-header">
			<h3 class="card-title">
				<i class="fas fa-users"></i> New Staff
			</h3>
		</div>
		<div class="card-body">
			<form class="form-horizontal" action="{{ route('staff.store') }}" method="POST">
				...
				<div class="form-group row {{ $errors->has('staffName') ? 'has-error' : '' }}">
					...
					<div class="col-sm-10">
						<input type="text" name="staffName" class="form-control @if($errors->has('staffName')) is-invalid @endif" value="{{ old('staffName') }}" required>
				...
				<div class="form-group row {{ $errors->has('staffTypeID') ? 'has-error' : '' }}">
					<label for="staffTypeID" class="col-sm-2 col-form-label">Staff Type: *</label>
					<div class="col-sm-10">
						<select name="staffTypeID" class="form-control select2 @if($errors->has('staffTypeID')) is-invalid @endif" required>
							...
							@foreach($staffTypes as $staffType)
								<option value="{{ $staffType->staffTypeID }}" {{ old('staffTypeID') == $staffType->staffTypeID ? 'selected' : '' }}>{{ $staffType->staffType }}</option>
							@endforeach
						</select>
				...
				<div class="form-group row {{ $errors->has('dateJoined') ? 'has-error' : '' }}">
					...
					<div class="col-sm-10">
	                    <input type="date" name="dateJoined" class="form-control @if($errors->has('dateJoined')) is-invalid @endif" value="{{ old('dateJoined') }}" required>
                ...
                <div class="form-group row {{ $errors->has('paymentFrequencyID') ? 'has-error' : '' }}">
					...
					<div class="col-sm-4">
						<select name="paymentFrequencyID" class="form-control select2 @if($errors->has('paymentFrequencyID')) is-invalid @endif" required>
                            ...
                            <option value="2" {{ old('paymentFrequencyID') == 2 ? 'selected' : '' }}>Daily</option>
                            <option value="1" {{ old('paymentFrequencyID') == 1 ? 'selected' : '' }}>Monthly</option>
						</select>
					...
					<div class="col-sm-4">
						<input type="number" name="paymentAmount" class="form-control" value="{{ old('paymentAmount') }}" required>
					</div>
				</div>
				<div>
